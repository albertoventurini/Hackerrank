...
Matrix gradient_descent(const Matrix& X, const Matrix& Y, const Matrix& init_theta, double alpha)
{
    ...
    while(true)
    {
        ...
        if( fabs(min(theta - prev_theta)) < 0.0001)
            break;
    }
...
Matrix matrix_transform(const Matrix& X, int order)
{
    int rows = X.get_n_rows();
    int cols = X.get_n_columns();
    
    int result_rows = rows;
    int result_cols = (cols - 1) * order + 1;
    ...
    for(int o = 1; o <= order; o++)
    {
        for(int i = 0; i < rows; i++)
            ...
            {
                int c = (o - 1) * (cols - 1) + j;
                result.elem(i, c) = pow(X.elem(i, j), (double)o);
            }
    }
...
int main()
{
    ...
    Matrix Z = matrix_transform(X, 1);
        
    Matrix Y = m.crop(0, m.get_n_columns() - 1, m.get_n_rows(), m.get_n_columns());
    
    Matrix init_theta = Matrix::ones(Z.get_n_columns(), 1);    
    Matrix theta = gradient_descent(Z, Y, init_theta, 0.05);
